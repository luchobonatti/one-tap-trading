...
///         - VerifyingPaymaster: switches from installModule to installValidations whitelist.
///           Kernel v3.1 uses installValidations(bytes21[],(uint32,address)[],bytes[],bytes[])
///           for validator installation — installModule(1, addr, bytes) reverts silently.
///         - SessionKeyValidator: re-deployed alongside to stay in sync.
///
///         Required env vars:
...
///           forge script script/DeployPhase2d.s.sol:DeployPhase2d \
///             --rpc-url $MEGAETH_RPC_URL \
///             --broadcast \
///             --legacy \
///             --gas-estimate-multiplier 5000
contract DeployPhase2d is Script {
